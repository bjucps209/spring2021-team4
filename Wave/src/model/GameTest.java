...
public class GameTest {
  ...
  public void testLoad_example1_expectResult() throws IOException {
    ...
    // assertTrue(game.getCurrentLevel().getAllObjects().size()-1 == 3); TODO: have
    // not implement powerUP yet, so it will be wrong
  ...
  public void testLoad_example2_expectResult() throws IOException {
     ...
     * levels.add(Wave.loadCustomLevel("level4")); Wave.getInstance().gameStart(new
     * ArrayList<Level>()); Game game = Wave.getInstance().getGame();
     */
    Wave.getInstance().loadAllUsers();
    Wave.getInstance().setCurrentUser(new User("AndrewTest"));
    Game game = new Game();
  ...
  public void testSave_middlleOfGameSingelPlayer_correctResult() {
    ...
    try (BufferedReader rd = new BufferedReader(new FileReader("SinglePlayerofMiddleGame.txt"))) {
      ...
      assertTrue(
          rd.readLine().equals("AndrewTest;130;30;20;SHIP1;350;380;5;5;200;200;PowerUp,TemporaryInvincible,5,1,-1"));
      assertTrue(rd.readLine().equals("3")); // should be 3 if add in powerup
      assertTrue(rd.readLine().equals("###gameobject"));
      assertTrue(rd.readLine().equals("EnemyObject;BOUNCER;20;20;5;5;4;4;60")); // TODO: test for speical affects later
                                                                                // on
      assertTrue(rd.readLine().equals("###gameobject"));
